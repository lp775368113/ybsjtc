...
public class ResultSetUtil {
	...
	public static void putEntryIgnoreNull(Map properties, ResultSetMetaData metaData,
		...
		if(columnName.indexOf("_")>0){
			columnNames[1] =  columnName.replaceAll("_", "");
		}
		...
		case Types.NUMERIC:
			...
			if (null != sField) {
				for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
					properties.put(columnNames[n], new java.math.BigDecimal(sField));
				}
...
//			if (null != datefield) {
//				for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
//					properties.put(columnNames[n], datefield);
//				}
//			}
//			break;
			java.sql.Timestamp timestampfiledtemp = resultSet.getTimestamp(i);
			if (null != timestampfiledtemp) {
				for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
					properties.put(columnNames[n], timestampfiledtemp);
				}
			}
			break;
		case Types.TIME:
			...
			if (null != timiefield) {
				for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
					properties.put(columnNames[n], timiefield);
				}
			}
			break;
		case Types.TIMESTAMP:
			...
			if (null != timestampfiled) {
				for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
					properties.put(columnNames[n], timestampfiled);
				}
			}
			break;
		case Types.CHAR:
		...
		case Types.VARCHAR: {
			...
			if (stemp == null)
				stemp = new String("");
			for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
				properties.put(columnNames[n], stemp);
			}
		}
			break;
		...
		case Types.VARBINARY:
			...
			if (null != stringfield) {
				for(int n=0; n<columnNames.length && columnNames[n]!=null; n++){
					properties.put(columnNames[n], stringfield);
				}
			}
			break;
